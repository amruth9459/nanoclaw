...
# ── Establish tonight's baseline ─────────────────────────────────────────────
# FIX 2026-07-15 (ratchet bug, diagnosed 2026-07-05): the old gate read
# BEST_F1 = max(new_f1) over ALL historical "kept" rows in results.tsv. Once
# any run ever hit 1.0, that max was pinned at 1.0 forever, so every later run
# was "discarded" and its experiment.py silently reverted. Cross-night F1s are
# also incomparable because the corpus (manifest.json, gitignored) mutates
# between nights. The gate now measures the CURRENT experiment.py on the
# CURRENT corpus once per session and compares only within tonight, with a
# doc-count tiebreak so equal-F1-on-a-larger-corpus counts as progress.
# Do NOT restore the historical-max read.
TOTAL_COST=0
BEST_DOCS=0
log "Measuring tonight's baseline (current experiment.py on current corpus)..."
rm -f "$SCRIPT_DIR/last-result.json"
python3 "$EXPERIMENT_FILE" > "$LOG_DIR/baseline-$(date +%Y%m%d-%H%M%S).log" 2>&1 || true
if [ -f "$SCRIPT_DIR/last-result.json" ]; then
    BASELINE_JSON=$(cat "$SCRIPT_DIR/last-result.json")
    rm -f "$SCRIPT_DIR/last-result.json"
    BEST_F1=$(echo "$BASELINE_JSON" | python3 -c "import sys,json; print(json.load(sys.stdin).get('overall_f1', 0))" 2>/dev/null || echo "0")
    BEST_DOCS=$(echo "$BASELINE_JSON" | python3 -c "import sys,json; print(len(json.load(sys.stdin).get('results', [])))" 2>/dev/null || echo "0")
    BASELINE_COST=$(echo "$BASELINE_JSON" | python3 -c "import sys,json; print(json.load(sys.stdin).get('total_cost_usd', 0))" 2>/dev/null || echo "0")
    TOTAL_COST=$(echo "$TOTAL_COST + $BASELINE_COST" | bc -l)
    log "Baseline: F1=$BEST_F1 on $BEST_DOCS docs (cost \$$BASELINE_COST)"
else
    log "WARNING: baseline run produced no last-result.json — gating against F1=0.0 (first successful experiment will be kept)"
fi
...
"
    ...
    if [ -n "$RESULT_JSON" ]; then
            ...
            # Compare with tonight's best (baseline-anchored; see fix note above).
            # Keep on strict F1 improvement, or on equal F1 with MORE docs scored
            # (corpus growth at held quality is progress, and F1 saturates at 1.0).
            PREV_BEST=$BEST_F1
            NEW_DOCS=$(echo "$RESULT_JSON" | python3 -c "import sys,json; print(len(json.load(sys.stdin).get('results', [])))" 2>/dev/null || echo "0")
            IS_IMPROVEMENT=$(echo "$NEW_F1 > $BEST_F1" | bc -l)
            IS_TIE=$(echo "$NEW_F1 == $BEST_F1" | bc -l)

            if [ "$IS_IMPROVEMENT" -eq 1 ] || { [ "$IS_TIE" -eq 1 ] && [ "$NEW_DOCS" -gt "$BEST_DOCS" ]; }; then
                log "IMPROVEMENT! F1 $PREV_BEST → $NEW_F1 (docs $BEST_DOCS → $NEW_DOCS) — keeping changes"
                BEST_F1=$NEW_F1
                BEST_DOCS=$NEW_DOCS
                STATUS="kept"
            ...
            # Append to results.tsv (prev_f1 = best BEFORE this experiment, so
            # the history reads honestly; the old code logged the updated value)
